...
void Signatures::FindAll() {
	...
	/* lua */
	pointerMap[Sigs::lua_absindex] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 8B ?? ?? 8D ?? ?? ?? ?? ?? 3D"
	);
	pointerMap[Sigs::lua_createtable] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 8B ?? ?? 8B ?? ?? ?? 78 ?? ?? 7E ?? 56 E8 ?? ?? ?? ?? 83 C4 ?? 56"
	);
	pointerMap[Sigs::lua_getfield] = Signatures::FindFirst(1,
		//Kong
		"55 8b ec ff 75 ?? ff 75 ?? ff 75 ?? e8 ?? ?? ?? ?? 83 c4 ?? 50 ff 75 ?? e8 ?? ?? ?? ?? 83 c4 ?? 5d c3 cc cc cc cc cc cc cc cc cc cc cc cc cc cc 55 8b ec 56 8b 75 ?? ff 75 ?? 6a ?? 8b 46 ?? 6a ?? ff 70 ?? e8 ?? ?? ?? ?? 83 c4 ?? 50 56 e8 ?? ?? ?? ?? 83 c4 ?? 5e 5d c3 cc cc cc cc cc cc cc 55 8b ec 53 56 8b 75 ?? 57 ff 75 ?? 56 e8 ?? ?? ?? ?? 8b 5d"
	);
	pointerMap[Sigs::lua_getglobal] = Signatures::FindFirst(1,
		//Kong
		"55 8b ec 56 8b 75 ?? ff 75 ?? 6a ?? 8b 46 ?? 6a ?? ff 70 ?? e8 ?? ?? ?? ?? 83 c4 ?? 50 56 e8 ?? ?? ?? ?? 83 c4 ?? 5e 5d c3 cc cc cc cc cc cc cc 55 8b ec 53 56 8b 75 ?? 57 ff 75 ?? 56 e8 ?? ?? ?? ?? 8b 5d"
	);
	pointerMap[Sigs::lua_newstate] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 53 8B ?? ?? 68"
	);
	pointerMap[Sigs::lua_pushcclosure] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 57 8B ?? ?? 85 FF 75 ?? 8B ?? ?? 8B ?? ?? 5F"
	);
	pointerMap[Sigs::lua_pushvalue] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 ?? ?? ?? 8B ?? ?? 56 E8 ?? ?? ?? ?? 8B ?? ?? 83 C4"
	);
	pointerMap[Sigs::lua_rawgeti] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 57 ?? ?? ?? 8B ?? ?? 57 E8 ?? ?? ?? ?? ?? ?? ?? 8B ?? ?? ?? ?? ?? ?? ?? E8"
	);
	pointerMap[Sigs::lua_rawlen] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 ?? 8B ?? ?? 83 E1 ?? 83 C1 ?? 83 F9 ?? 77 ?? 0F ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 8B ?? 0F"
	);
	pointerMap[Sigs::lua_rawseti] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 8B ?? ?? 57 ?? ?? ?? 56 E8 ?? ?? ?? ?? 8B ?? ?? 8B F8 83 E9 ?? 51 ?? ?? ?? ?? ?? ?? ?? ?? 56"
	);
	pointerMap[Sigs::lua_settop] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 8B ?? ?? 8B ?? ?? 8B ?? 8B ?? ?? 85"
	);
	pointerMap[Sigs::lua_type] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 ?? 3D C8 ?? ?? ?? 74"
	);
	pointerMap[Sigs::lua_version] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 8B ?? ?? 85 C0 75 ?? B8 88"
	);
	pointerMap[Sigs::luaC_barrierback_] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 8B ?? ?? 8B ?? ?? 8B ?? ?? ?? 61"
	);
	pointerMap[Sigs::luaC_step] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 53 8B ?? ?? 56 57 8B ?? ?? 57 E8"
	);
	pointerMap[Sigs::luaH_new] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 6A ?? 6A"
	);
	pointerMap[Sigs::luaH_resize] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 EC ?? 53 8B ?? ?? 56 8B ?? ?? 57 0F"
	);
	pointerMap[Sigs::luaH_setint] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 EC ?? 56 8B ?? ?? 57 8B ?? ?? 56 57 ?? ?? ?? E8"
	);
	pointerMap[Sigs::luaL_argerror] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 EC ?? 8D ?? ?? 57"
	);
	pointerMap[Sigs::luaL_checktype] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B ?? ?? 83 C4 ?? 3B"
	);
	pointerMap[Sigs::luaL_checkversion_] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 8B ?? ?? 57 56 E8 ?? ?? ?? ?? 83 C4 ?? 8B"
	);
	pointerMap[Sigs::luaL_error] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 56 8B ?? ?? 6A ?? 56 E8 ?? ?? ?? ?? 8D"
	);
	pointerMap[Sigs::luaL_openlibs] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC B8 50 ?? ?? ?? 56"
	);
	pointerMap[Sigs::luaL_setfuncs] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 51 53 8B ?? ?? 56 8B ?? ?? 57 68"
	);
	pointerMap[Sigs::luaN_openlibs] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 83 EC ?? A1 34 ?? ?? ?? 33 C4 ?? 44 ?? ?? 56 57 8B"
	);
	pointerMap[Sigs::luaN_register_searcher] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 51 56 57 8B F1 8B"
	);
	pointerMap[Sigs::luaN_table_insert] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 EC ?? 53 56 57 8B F9 6A"
	);
	pointerMap[Sigs::luaN_tobaseobject] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 ?? 8B ?? ?? 83 E1 ?? 83 E9"
	);
	pointerMap[Sigs::luaNopen_analytics] = Signatures::FindFirst(1,
		//Kong
		"?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 56 e8 ?? ?? ?? ?? 6a ?? 6a ?? 56 e8 ?? ?? ?? ?? 83 c4 ?? 8d 44 24 ?? 6a ?? 50 56 e8 ?? ?? ?? ?? 83 c4 ?? b8 ?? ?? ?? ?? 5e 8b e5 5d c3 cc 55 8b ec 83 e4 ?? a1"
	);
	pointerMap[Sigs::luaNopen_animation] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 83 EC ?? A1 34 ?? ?? ?? 33 C4 ?? 44 ?? ?? F2 0F ?? ?? ?? ?? ?? ?? 56 8B ?? ?? 68 ?? ?? ?? ?? 83 EC ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 0F ?? ?? ?? 56 ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? E8"
	);
	/*pointerMap[Sigs::luaNopen_assets] = Signatures::FindFirst(1,
		//Kong
		""
	);
	pointerMap[Sigs::luaNopen_loc] = Signatures::FindFirst(1,
		//Kong
		""
	);
	pointerMap[Sigs::luaNopen_special_events] = Signatures::FindFirst(1,
		//Kong
		""
	);*/
	pointerMap[Sigs::luaNopen_time] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 83 EC ?? F2 0F ?? ?? ?? ?? ?? ?? 56 8B ?? ?? 68 ?? ?? ?? ?? 83 EC ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 0F ?? ?? ?? 56 E8 ?? ?? ?? ?? 6A ?? 6A ?? 56 E8 ?? ?? ?? ?? 6A"
	);
	pointerMap[Sigs::luaNopen_transitions] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 83 EC ?? A1 34 ?? ?? ?? 33 C4 ?? 44 ?? ?? F2 0F ?? ?? ?? ?? ?? ?? 56 8B ?? ?? 68 ?? ?? ?? ?? 83 EC ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 0F ?? ?? ?? 56 ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44"
	);
	pointerMap[Sigs::luaNopen_ui] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 81 EC ?? ?? ?? ?? A1 34 ?? ?? ?? 33 C4 ?? 84 ?? ?? ?? ?? ?? F2"
	);
	pointerMap[Sigs::luaNopen_window] = Signatures::FindFirst(1,
		//Kong
		"55 8B EC 83 E4 ?? 83 EC ?? A1 34 ?? ?? ?? 33 C4 ?? 44 ?? ?? F2 0F ?? ?? ?? ?? ?? ?? 56 8B ?? ?? 68 ?? ?? ?? ?? 83 EC ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? ?? 0F ?? ?? ?? 56 ?? 44 ?? ?? ?? ?? ?? ?? ?? 44 ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 6A ?? 6A ?? 56 E8 ?? ?? ?? ?? 6A"
	);
	/* Screens */
