...
void Signatures::FindAll() {
	...
	pointerMap[Sigs::CBaseScreen_GetScreen] = Signatures::FindFirst(1,
	...
	/* CMainButton */
	pointerMap[Sigs::CMainButton_CCTOR] = Signatures::FindFirst(1,
	...
	/* CSettingsScreen */
	pointerMap[Sigs::CSettingsScreen_VTable] = h_rtti::get_vtable_pointer("CSettingsScreen");
	...
	pointerMap[Sigs::CSettingsScreen_Process] = Signatures::FindFirst(1,
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 ?? ?? ?? ?? ?? 50 83 EC ?? A1 34 ?? ?? ?? 33 C5 ?? 45 ?? 56 57 50 8D ?? ?? ?? A3 ?? ?? ?? ?? 8B F1 8B ?? ?? 50 E8 ?? ?? ?? ?? ?? BE"
	);
	/* CSprite */
	...
	/* SMainButtonStyle */
	pointerMap[Sigs::SMainButtonStyle_CCTOR] = Signatures::FindFirst(1,
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 ?? ?? ?? ?? ?? 50 51 56 57 A1 34 ?? ?? ?? 33 C5 50 8D ?? ?? ?? A3 ?? ?? ?? ?? 8B F1 ?? 75 ?? ?? 06"
	);
	pointerMap[Sigs::SMainButtonStyle_CopyCCTOR] = Signatures::FindFirst(1,
