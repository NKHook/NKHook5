...
namespace NKHook5
{
    namespace Patches
    {
        namespace CSettingsScreen
        {
            ...
            static void __fastcall cb_hook(ClassesEx::CSettingsScreenExt* self, int pad, Classes::SGameTime* pGameTime) {
            }

            auto Input::Apply() -> bool
            {
                const void* address = Signatures::GetAddressOf(Sigs::CSettingsScreen_Input);
                if(address)
                {
