...
namespace NKHook5::Patches::CFlagStringConvertor
{
	...
	nfw::string FlagToString::cb_hook(uint32_t categoryId, uint64_t numericId) {
		auto ofn = std::bit_cast<decltype(&FlagToString::cb_hook)>(reinterpret_cast<void*>(o_func));
		nfw::string resultStr = (this->*ofn)(categoryId, numericId);
		//Code for tower id injections
		if (this == reinterpret_cast<void*>(static_cast<Classes::CFlagStringConvertor*>(g_towerFactory))) {
			if (categoryId == 0) {
		...
		if (this == reinterpret_cast<void*>(static_cast<Classes::CFlagStringConvertor*>(g_bloonFactory)))
		{
		...
		if (this == reinterpret_cast<void*>(static_cast<Classes::CFlagStringConvertor*>(g_weaponFactory)))
		{
	...
	auto FlagToString::Apply() -> bool
	{
		...
		if(address)
		{
			auto* detour = new PLH::x86Detour((const uint64_t)address, std::bit_cast<size_t>(&FlagToString::cb_hook), &o_func);
			if(detour->hook())
			{
				return true;
			}
